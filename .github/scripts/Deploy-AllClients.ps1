    ...
    try {
        ...
        try {
            ...
            foreach ($batch in $batches) {
                # Strip sqlcmd-mode directives (lines starting with ':' e.g. :setvar, :on error,
                # :connect). These are generated by sqlpackage but are NOT valid T-SQL and will
                # cause "Incorrect syntax near ':'" when sent through SqlCommand.
                $cleanBatch = ($batch -split '\r?\n' |
                    Where-Object { $_ -notmatch '^\s*:' }) -join "`n"

                if ([string]::IsNullOrWhiteSpace($cleanBatch)) { continue }

                $batchCmd = $trialConn.CreateCommand()
                $batchCmd.CommandText = $cleanBatch
